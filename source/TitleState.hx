...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('datree/MenuUI/titleBG'));
		bg.antialiasing = ClientPrefs.globalAntialiasing;
		add(bg);

		logoBl = new FlxSprite(85, 10);
		logoBl.frames = Paths.getSparrowAtlas('datree/MenuUI/logobumpin');
		...
		cake = new FlxSprite(0, FlxG.height * 0.4).loadGraphic(Paths.image('hitshard'));
		add(cake);
		cake.visible = false;
		cake.setGraphicSize(Std.int(cake.width * 0.55));
		cake.updateHitbox();
		cake.screenCenter(X);
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 20:
					deleteCoolText();
				case 21:
					createCoolText(['This image hits hard'], 15);
				case 23:
					cake.visible = true;
				case 24:
					deleteCoolText();

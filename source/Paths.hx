...
class Paths
{
	...
	static public function preloadImage(key:String):FlxGraphic {
		if(FileSystem.exists(image(key,'shared'))) {
			if(!customImagesLoaded.exists(key)) {
				var newBitmap:BitmapData = BitmapData.fromFile(image(key,'shared'));
